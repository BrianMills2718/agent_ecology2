...
| 41 | Agent Hospital (SEAL paradigm) | **Read** |
| 42 | MedAgentSim (MICCAI 2025) | **Read** |
| 43 | MetaAgent (Tool Meta-Learning) | **Read** |
...
### Agent Hospital: Simulacrum-based Evolutionary Agent Learning (SEAL)

Source: https://arxiv.org/abs/2405.02957 (Tsinghua University)

**Core Insight: Two Phases of Medical Expertise**
1. **Knowledge acquisition** - Learning from textbooks (what LLMs already do)
2. **Expertise acquisition** - Learning from practice (what Agent Hospital enables)

**SEAL Architecture:**

```
Simulacrum Construction:
  - 42 doctor agents, 4 nurse agents, unlimited patient agents
  - 32 departments covering 339 diseases
  - Patient agents auto-generated by LLM + medical knowledge base
  - Full hospital workflow: triage → registration → consultation →
    examination → diagnosis → pharmacy → convalescence → follow-up

Agent Evolution (MedAgent-Zero):
  - Medical Case Base: Store successful treatment cases
  - Experience Base: Rules extracted from failures via reflection
  - Both grow over time as more patients are treated
```

**Key Mechanism - Learning from Failure:**
```
1. Doctor diagnoses patient incorrectly
2. Compare with ground truth
3. Reflect: "Why did I fail? What rule would prevent this?"
4. Generate candidate rule
5. Test rule on same patient
6. If successful → add to Experience Base
7. If not → discard rule
```

**Scaling Laws in Evolution:**
- After treating ~10,000 patients (equivalent to 2+ years human practice)
- Doctor agents achieve **93.06%** on MedQA respiratory subset
- Performance keeps improving with more patients treated
- **Virtual world expertise transfers to real-world benchmarks**

**Three Evaluation Tasks:**
1. Medical examination selection (which tests to order)
2. Diagnosis (identify the disease)
3. Treatment plan recommendation (what to prescribe)

**Why This Matters for Us:**
- Demonstrates that agents CAN learn from simulated experience without labeled data
- Case base + Experience base = two types of memory we should consider
- Reflection on failure is key to learning (not just storing successes)
- The "virtual to real" transfer suggests simulation-based training is viable

**Relevance:** Our agents could use a similar dual-memory approach: store successful artifact operations (case base) and extract rules from failures (experience base). The simulation-based learning approach could help agents develop better heuristics over time without requiring human-labeled training data.

---

### MedAgentSim: Self-Evolving Multi-Agent Clinical Simulations

Source: https://arxiv.org/abs/2503.22678 (MICCAI 2025 Oral)

**Key Innovation: Doctor Starts with NO Prior Knowledge**

Unlike prior approaches where doctor agents receive complete patient info upfront, MedAgentSim requires:
- Multi-turn conversation to gather symptoms
- Explicit requests for tests (temp, BP, ECG, imaging)
- Active information gathering before diagnosis

**Three Agent Types:**
1. **Patient Agent** - Has symptoms, responds to questions
2. **Doctor Agent** - Must actively investigate, no info given upfront
3. **Measurement Agent** - Provides test results ONLY when requested

**Two-Phase Architecture:**

```
Phase 1: Conversation Phase
  - Doctor questions patient
  - Doctor requests specific tests from Measurement Agent
  - Results only provided if explicitly requested
  - Mimics real clinical consultation

Phase 2: Experience Replay Phase
  - KNN retrieves similar past cases
  - Multi-agent discussion (multiple doctor agents)
  - Chain-of-Thought reasoning
  - Majority-vote ensembling → final diagnosis
```

**Dual Memory Buffers:**
1. **Medical Records Buffer** - Correctly diagnosed cases (full conversation + diagnosis)
2. **Experience Records Buffer** - Misdiagnosed cases that were CORRECTED after reflection

**Learning from Corrected Mistakes:**
```
1. Initial diagnosis is wrong
2. Doctor reflects: "What did I miss?"
3. Doctor attempts second diagnosis
4. If correct → store ONLY the reflection insights (not full case)
5. If still wrong → discard entirely
```

**Results:**
| Benchmark | Baseline | MedAgentSim | Improvement |
|-----------|----------|-------------|-------------|
| NEJM | 20.0% | 26.7% | +6.7% |
| MedQA | 62.3% | 70.8% | +8.5% |
| MIMIC-IV | 42.7% | **79.5%** | **+36.8%** |

**Key Ablation Findings:**
- Adding measurement agent: +improvement
- Adding memory: +improvement
- Adding CoT: +improvement
- Adding ensembling: +improvement
- All combined (LLaMA 3.3): **+16.1%** total boost

**Human Study:** Doctors watching videos failed to identify the human participant 62.5% of the time → high behavioral realism.

**Relevance:** The "start with nothing, actively gather" model is exactly what we need. Our agents should not receive complete world state upfront - they should query for what they need. The dual buffer pattern (successes vs corrected failures) is more nuanced than simple case storage. The massive MIMIC-IV improvement (+36.8%) suggests this approach works especially well on realistic clinical scenarios.

---

### MetaAgent: Self-Evolving via Tool Meta-Learning

Source: https://arxiv.org/abs/2508.00271

**Core Paradigm: Learning by Doing**

MetaAgent starts with minimal capabilities and builds expertise through experience - no parameter updates required.

**Starting State (Minimal Workflow):**
- Basic reasoning ability
- Adaptive help-seeking (knows when to ask for help)
- Tool router (routes requests to appropriate external tools)

**Evolution Mechanism:**

```
When knowledge gap encountered:
  1. Generate natural language help request
  2. Tool router selects appropriate external tool
  3. Execute tool, get result
  4. Self-reflect on result
  5. Verify answer quality
  6. Distill experience into concise text
  7. Add to persistent knowledge base
```

**Meta Tool Learning (Key Concept):**
- NOT fine-tuning the model
- NOT updating parameters
- Instead: organizing interaction history into reusable knowledge
- Building "in-house tools" from experience
- Knowledge base grows more informative over time

**What Gets Stored:**
- Tool-use strategies that worked
- Reasoning patterns that led to correct answers
- Condensed text snippets capturing key insights

**Results on Knowledge Discovery:**
- Outperforms workflow-based baselines on GAIA, WebWalkerQA, BrowseComp
- Matches or exceeds end-to-end trained agents
- Achieves this WITHOUT model updates

**Key Differentiator from Other Approaches:**
- Agent Hospital: Evolution via case base + experience base
- MedAgentSim: Evolution via memory buffers + reflection
- MetaAgent: Evolution via **tool meta-learning** + knowledge base construction

**Why This Matters:**

The "autonomously builds in-house tools" concept is powerful:
```
Traditional: Agent uses predefined tools
MetaAgent: Agent creates NEW tools from experience

Example:
  - Agent repeatedly does complex calculation
  - Agent notices pattern
  - Agent creates reusable "calculation helper" tool
  - Future tasks can use this self-created tool
```

**Relevance:** Our agents could potentially create their own genesis-style artifacts as tools. Instead of us pre-defining all genesis services, agents could build reusable services from patterns they discover. This aligns with our "emergence over prescription" philosophy - let agents discover and codify useful patterns themselves.
