def run():
    ...
    # Read current state
    state_raw = kernel_state.read_artifact("discourse_analyst_3_state", caller_id)
    try:
    ...
    strategy = kernel_state.read_artifact("discourse_analyst_3_strategy", caller_id)
    if not strategy:
        strategy = "You are a rhetoric analyst."
...
- Write artifact (tool): {{"action_type": "write_artifact", "artifact_id": "discourse_analyst_3_tool_X", "artifact_type": "executable", "executable": true, "code": "def run(text):\\n    # analyze text\\n    return result"}}
- Write artifact (data): {{"action_type": "write_artifact", "artifact_id": "discourse_analyst_3_data_X", "artifact_type": "json", "content": {{...}}}}
...
"""
    ...
    if not llm_result.get("success"):
        ...
        kernel_actions.write_artifact(caller_id, "discourse_analyst_3_state", json.dumps(state, indent=2))
