# Discourse V3 Genesis Agent (Cross-Agent Smoke Test)

Task-driven research agent focused on argument & logic. Same domains as V2,
...
| `strategy.md` | System prompt with argument & logic focus + cross-agent guidance |
...
4-artifact cluster:
1. **discourse_v3_strategy** - Text artifact with system prompt
2. **discourse_v3_state** - JSON artifact with task queue + research state
3. **discourse_v3_notebook** - JSON artifact with persistent long-term memory
4. **discourse_v3_loop** - Executable artifact with `has_loop=True`
